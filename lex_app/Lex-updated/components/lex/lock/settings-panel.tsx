...
import {
  ...
  Sparkles, ChevronRight, ImagePlus,
} from "lucide-react"
import {
...
function PremiumGate({
  ...
  const planEmoji: Record<string, string> = { pro: "⚡", plus: "🚀", ultra: "👑" }
    ...
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span
            className="shrink-0 text-[9px] font-bold rounded-full px-1.5 py-0.5 border"
            style={{ color: planColors[plan], borderColor: planColors[plan] + "60" }}
          >
            {planEmoji[plan]} {plan.toUpperCase()}
          </span>
        </div>
...
export function LockScreenSettingsPanel({
  ...
  function AnimationPicker() {
    const options: { val: UnlockAnimation; label: string; emoji: string }[] = [
      { val: "slide",  label: "Slide",  emoji: "⬆️" },
      { val: "fade",   label: "Fade",   emoji: "✨" },
      { val: "scale",  label: "Scale",  emoji: "🔍" },
      { val: "spring", label: "Spring", emoji: "🌀" },
    ]
    return (
      <div className="px-3 py-3">
