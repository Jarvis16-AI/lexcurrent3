    ...
    if (type === "patterns") {
      const rows = await query<{ action: string; count: number; last_used: string; hour_of_day: number }>(
        `SELECT action, count, last_used, hour_of_day FROM usage_patterns
         WHERE user_id = $1 ORDER BY count DESC LIMIT 20`, [userId]
      )
  ...
  try {
    ...
    if (type === "settings") {
      ...
      await query(
        ...
        [userId, daily_limit_minutes ?? 120,
          `{${(focus_apps ?? []).join(",")}}`,
    ...
    if (type === "usage") {
      ...
      if (existing.length > 0) {
        await query(
          `UPDATE usage_patterns SET count = count + 1, last_used = NOW(), hour_of_day = $3, day_of_week = $4 WHERE id = $1`,
          [existing[0].id, action, hour_of_day ?? new Date().getHours(), day_of_week ?? new Date().getDay()]
        )
      } else {
        await query(
          `INSERT INTO usage_patterns (user_id, action, hour_of_day, day_of_week) VALUES ($1, $2, $3, $4)`,
          [userId, action, hour_of_day ?? new Date().getHours(), day_of_week ?? new Date().getDay()]
        )
      }
      return NextResponse.json({ ok: true })
    }
