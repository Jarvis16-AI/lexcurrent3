...
export async function GET(req: NextRequest) {
  ...
  try {
    await ensureTables()
    const rows = await query<{ id: number; category: string; content: string; confidence: number; reinforcement_count: number; last_reinforced: string; created_at: string }>(
      `SELECT id, category, content, confidence, reinforcement_count, last_reinforced, created_at
       FROM memories WHERE user_id = $1
       ORDER BY (confidence * 0.7 + LEAST(reinforcement_count, 10) * 0.03) DESC, last_reinforced DESC
       LIMIT 60`,
      [userId]
    )
