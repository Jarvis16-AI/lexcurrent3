...
import { X, Cloud, Bell, Sparkles } from "lucide-react"
import type { Weather } from "./types"
import { cn } from "@/lib/utils"

export interface Notification {
  id: string
  type: "lex" | "weather" | "reminder" | "system"
  title: string
  body: string
  time: string
  read: boolean
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-2">
          {notifs.length === 0 ? (
            <div className="flex flex-col items-center py-10 text-muted-foreground">
              ...
              <p className="text-xs opacity-50 mt-1">Only real notifications appear here</p>
            </div>
          ) : (
            notifs.map(n => (
              <div
...
/* ── Only real system notifications — no fake messages ── */
export function buildNotifications(weather: Weather | null): Notification[] {
  ...
  const base: Notification[] = [
    {
      ...
      type: "lex",
      title: "LEX",
      body:  "LEX is active. Tap the orb or ask me anything.",
      time:  t(1),
      read:  false,
    },
  ]
  ...
  if (weather) {
    base.push({
      ...
      body:  `${weather.temp}${weather.unit} and ${weather.label}. H ${weather.high}° · L ${weather.low}°.`,
      time:  t(5),
      read:  false,
    })
  }
