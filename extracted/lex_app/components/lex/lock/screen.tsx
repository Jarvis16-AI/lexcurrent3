...
function ContextCard({ time, blur, bg }: { time: Date | null; blur: string; bg: string }) {
  ...
  const greeting =
    h < 5  ? "🌙 Late night, stay focused" :
    h < 12 ? "🌅 Good morning, let's do this" :
  ...
  return (
    <div className={cn("rounded-2xl border border-white/15 px-4 py-3", blur, bg)}>
      ...
      <p className="text-sm text-white font-medium">{greeting}</p>
        ...
        <div className="relative flex h-full flex-col justify-between py-10 px-5">
          ...
          {ls.quickApps && (
            <>
              ...
              >
                <span className={cn("text-xl leading-none", torchOn ? "text-yellow-300" : "text-white/70")}>🔦</span>
