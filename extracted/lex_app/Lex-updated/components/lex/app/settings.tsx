...
import {
  ...
  KeyRound, ShieldCheck, ShieldOff, Terminal,
} from "lucide-react"
import { LockScreenSettingsPanel } from "../lock/settings-panel"
import type { AppShared } from "./types"
import type { AppSettings, Wallpaper, Theme, FontSize, AIPersonality } from "@/lib/settings"
import {
  WALLPAPERS, ACCENT_PRESETS, FALLBACK_VOICES,
  applyTheme, applyAccent, applyFontSize,
} from "@/lib/settings"
import { cn } from "@/lib/utils"
import { setPremium, clearPremium, isPremium, getPremiumTier, type PremiumTier } from "@/lib/quota"
  ...
  return (
    <button
      ...
      className={cn(
        ...
        onPress ? "hover:bg-accent/50 active:bg-accent/70 transition-colors" : "cursor-default",
      )}
    >
      <div>
  ...
  async function redeemBypassCode() {
    ...
    try {
      ...
      if (data.valid) {
        ...
        setBypassResult({ ok: false, msg: `❌ ${data.error ?? "Invalid code."}` })
      }
    } catch {
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-8">
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
          ...
          <div className="border-t border-border/50" />
          <SettingRow label="Powered by" sub="Groq · ElevenLabs · Next.js 16 · Tailwind CSS 4" />
          <div className="border-t border-border/50" />
          <SettingRow
