...
export type FontFamily    = "geist" | "inter" | "mono" | "serif"
export type Wallpaper     = "amber" | "ocean" | "forest" | "violet" | "rose" | "graphite" | "midnight" | "amoled" | "aurora" | "sunset"
export type AIPersonality = "friendly" | "professional" | "concise"
export type AIModelId    = "lex-flash" | "lex-think" | "lex-reason"
export type OrbSpeed     = "calm" | "normal" | "dynamic"
export type ColorTheme   = "default" | "midnight" | "amoled" | "ocean" | "aurora" | "sunset" | "material"
...
export const WALLPAPERS: Record<Wallpaper, { label: string; gradient: string; preview: [string, string] }> = {
  ...
  midnight: { label: "Midnight",  gradient: "radial-gradient(ellipse at top, rgba(15,23,42,0.9) 0%, #000010 60%)",  preview: ["#0f172a", "#000010"] },
  amoled:   { label: "AMOLED",    gradient: "radial-gradient(ellipse at top, rgba(0,0,0,1) 0%, #000000 100%)",       preview: ["#111111", "#000000"] },
  aurora:   { label: "Aurora",    gradient: "radial-gradient(ellipse at top, rgba(6,78,59,0.4) 0%, rgba(46,16,101,0.4) 50%, #050010 80%)", preview: ["#064e3b", "#2e1065"] },
  sunset:   { label: "Warm Glow", gradient: "radial-gradient(ellipse at top, rgba(124,45,18,0.5) 0%, rgba(136,19,55,0.3) 50%, #0c0a09 80%)", preview: ["#7c2d12", "#881337"] },
}

/* ── color themes ────────────────────────────────────────────── */
export const COLOR_THEMES: { key: ColorTheme; label: string; emoji: string; description: string }[] = [
  { key: "default",  label: "LEX Default", emoji: "🔥", description: "Warm amber signature" },
  { key: "midnight", label: "Midnight",    emoji: "🌃", description: "Deep navy blue" },
  { key: "amoled",   label: "AMOLED",      emoji: "⬛", description: "True black display" },
  { key: "ocean",    label: "Ocean",       emoji: "🌊", description: "Cool deep blue" },
  { key: "aurora",   label: "Aurora",      emoji: "🌌", description: "Northern lights" },
  { key: "sunset",   label: "Sunset",      emoji: "🌅", description: "Warm red-orange" },
  { key: "material", label: "Material You",emoji: "🎨", description: "Adaptive Google style" },
]
...
export const ACCENT_PRESETS: { key: string; label: string; oklch: string; hex: string }[] = [
  ...
  { key: "pink",    label: "Pink",    oklch: "oklch(0.7 0.2 340)",   hex: "#db2777" },
  { key: "indigo",  label: "Indigo",  oklch: "oklch(0.55 0.25 280)", hex: "#4338ca" },
]

/* ── font families ───────────────────────────────────────────── */
export const FONT_FAMILIES: { key: FontFamily; label: string; sample: string; stack: string }[] = [
  { key: "geist",  label: "Geist",     sample: "Aa", stack: "var(--font-geist-sans), sans-serif"   },
  { key: "inter",  label: "Inter",     sample: "Aa", stack: "'Inter', 'SF Pro Display', sans-serif" },
  { key: "mono",   label: "Monospace", sample: "01", stack: "var(--font-geist-mono), monospace"     },
  { key: "serif",  label: "Serif",     sample: "Aa", stack: "'Georgia', serif"                      },
]
...
export function applyTheme(theme: Theme) {
  ...
  else if (theme === "light")  root.classList.remove("dark"), root.classList.add("light")
...
export function applyFontFamily(family: FontFamily) {
  if (typeof window === "undefined") return
  const f = FONT_FAMILIES.find(f => f.key === family) ?? FONT_FAMILIES[0]
  document.documentElement.style.setProperty("--app-font-family", f.stack)
}

export function applyOrbSpeed(speed: OrbSpeed) {
  if (typeof window === "undefined") return
  const map: Record<OrbSpeed, string> = { calm: "1.8", normal: "1", dynamic: "0.55" }
  document.documentElement.style.setProperty("--orb-speed-mult", map[speed])
}
...
export function applyAllSettings(s: AppSettings) {
  ...
  applyFontFamily(s.fontFamily ?? "geist")
  applyOrbSpeed(s.orbSpeed ?? "normal")
}
